...
# Reproducibility used to be checked here via a scratch-copy re-run of the
# whole extractor (including a second OCR pass) — correct, but expensive
# enough that it hung the first CI run for 15+ minutes on a shared runner.
# CI now proves the same thing more cheaply, the same way it already does
# for coverage.md/COMPENDIUM.md: re-run scripts/extract_pdf_text.py
# directly against the real checkout and `git diff --exit-code -- sources
# docs`. See the "Regenerate derived files and diff" step in
